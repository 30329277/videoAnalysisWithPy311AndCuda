...
def process_frame(frame, model, transform, device, threshold=0.8):
    # 将帧转换为模型所需的格式并移动到GPU
...
def detect_objects_in_video(video_path, model, transform, device, num_threads=4, threshold=0.8):
    cap = cv2.VideoCapture(video_path)
    ...
    def worker(frame_queue):
        ...
        while True:
            try:
                ...
                object_name = process_frame(frame, model, transform, device, threshold)
                if object_name:
                    with lock:
...
if __name__ == "__main__":
    ...
    detected_frames, video_fps = detect_objects_in_video(video_path, model, transform, device, num_threads=4)
